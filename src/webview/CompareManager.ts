...
import { compareDirectories, copyDirectoryPair, copyMetadataField, deleteDirectoryPair, syncTargetCase, DirectoryComparison, FileMetadata, MetadataField, PlatformCapabilities } from '../core/DirectoryDiff';
...
export class CompareManager {
  ...
  private constructor(private readonly context: vscode.ExtensionContext) {
    ...
    this.configurationListener = vscode.workspace.onDidChangeConfiguration((event: vscode.ConfigurationChangeEvent) => {
      if (!event.affectsConfiguration('codemeld.appearance')) return;
  ...
  private createPanel(): vscode.WebviewPanel {
    ...
    panel.webview.html = managerHtml(panel.webview, monacoVsUri, appearance);
    return panel;
  }
  ...
  private async refreshDirectory(session: DirectorySession): Promise<void> {
    try {
      session.comparison = await compareDirectories(session.leftUri, session.rightUri);
      await this.panel.webview.postMessage({ type: 'directoryUpdated', sessionId: session.id, comparison: session.comparison });
      void vscode.window.setStatusBarMessage('CodeMeld: directory comparison refreshed', 1400);
    } catch (error) {
  ...
  private async transferMetadata(session: MetadataSession, field: MetadataField, targetSide: 'left' | 'right'): Promise<void> {
    ...
    try {
      ...
      if (parent?.type === 'directory') {
        parent.comparison = await compareDirectories(parent.leftUri, parent.rightUri);
...
function isMessage(value: unknown): value is WebviewMessage {
  ...
  return type === 'ready' || type === 'contentChanged' || type === 'recompute' || type === 'save' ||
    type === 'closeSession' || type === 'activateSession' || type === 'appearance' || type === 'directoryMerge' || type === 'directoryDeleteUnmatched' || type === 'refreshDirectory' || type === 'openDirectoryPair' || type === 'openDirectoryPairModes' || type === 'openDirectoryPairAuto' || type === 'copyMetadata';
}

function managerHtml(webview: vscode.Webview, monacoVsUri: vscode.Uri, appearance?: AppearanceSettings): string {
  const payload = JSON.stringify({ monacoVsUri: monacoVsUri.toString(), appearance }).replace(/</g, '\\u003c');
  const csp = [
    "default-src 'none'",
...
.dirMergeControls{display:flex;gap:4px;justify-self:center}.dirRefreshSlot{justify-self:end}.dirMergeButton{width:30px;height:28px;display:grid;place-items:center;border:1px solid transparent;border-radius:8px;background:transparent;color:color-mix(in srgb,var(--codemeld-blue) 75%,white);cursor:pointer}.dirMergeButton.deleteButton{color:color-mix(in srgb,var(--codemeld-delete) 78%,white)}.dirMergeButton:disabled{opacity:.22;cursor:default}.dirMergeButton:not(:disabled):hover{background:color-mix(in srgb,var(--codemeld-blue) 16%,transparent);border-color:color-mix(in srgb,var(--codemeld-blue) 45%,transparent)}.dirMergeButton.deleteButton:not(:disabled):hover{background:color-mix(in srgb,var(--codemeld-delete) 14%,transparent);border-color:color-mix(in srgb,var(--codemeld-delete) 42%,transparent)}.dirMergeButton svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
.dirGrid{flex:1;min-height:0;display:grid;grid-template-columns:minmax(0,1fr) 72px minmax(0,1fr)}.dirPane{min-width:0;display:flex;flex-direction:column;overflow:hidden}.dirPath{height:34px;flex:0 0 34px;display:flex;align-items:center;padding:0 11px;border-bottom:1px solid rgba(255,255,255,.08);font-size:11px;color:var(--vscode-descriptionForeground);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.dirCenter{min-width:0;display:flex;flex-direction:column;background:var(--codemeld-gutter);border-left:1px solid rgba(255,255,255,.07);border-right:1px solid rgba(255,255,255,.07)}.dirCenterHeader{height:34px;flex:0 0 34px;border-bottom:1px solid rgba(255,255,255,.08)}.dirCenterRows{flex:1;min-height:0;overflow:hidden;font-size:12px}.dirRows{flex:1;min-height:0;overflow:auto;font-size:12px;scrollbar-width:thin}.dirRows::-webkit-scrollbar{width:8px}.dirRows::-webkit-scrollbar-thumb{background:rgba(255,255,255,.13);border-radius:8px}
...
.dirEmpty{padding:18px;color:var(--vscode-descriptionForeground);font-size:12px}
/* Metadata comparison */
...
function syncDirScroll(s,a,b,center){if(a.dataset.syncBound)return;a.dataset.syncBound='1';a.addEventListener('scroll',()=>{if(s.dirSync)return;s.dirSync=true;b.scrollTop=a.scrollTop;center.style.transform='translateY(' + (-a.scrollTop) + 'px)';requestAnimationFrame(()=>s.dirSync=false)})}
